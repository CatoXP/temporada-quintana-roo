...
(function () {
  ...
  function agua(host) {
    ...
    // peces en cardúmenes
    const CARDUMENES = [
      { cx: 5, cz: -4.2, ax: 3.2, az: 1.8, w: 0.17, y: -0.8, color: 0xffc93c },
      { cx: 6.5, cz: -7, ax: 4, az: 2.4, w: -0.13, y: -1.2, color: 0x3fb8ff },
      { cx: 3, cz: -3.2, ax: 3.5, az: 1.2, w: 0.21, y: -0.65, color: 0xff7a45 },
      { cx: -4, cz: -8, ax: 5, az: 2.5, w: 0.11, y: -1.3, color: 0xfff1b0 },
      { cx: 8, cz: -3.8, ax: 2.5, az: 1.4, w: -0.24, y: -0.9, color: 0x2ee6c5 },
    ];
    const N = 90;
    const peces = new THREE.InstancedMesh(geoPez(), new THREE.MeshLambertMaterial({ side: THREE.DoubleSide, emissive: 0x0d3035 }), N);
    const datosPez = [];
    for (let i = 0; i < N; i++) {
      const c = CARDUMENES[i % CARDUMENES.length];
      datosPez.push({ c, ox: (rnd() - 0.5) * 2.2, oy: (rnd() - 0.5) * 0.35, oz: (rnd() - 0.5) * 1.4, f: rnd() * TAU, e: 1.0 + rnd() * 0.6, hx: 0, hz: 0 });
      const col = new THREE.Color(c.color).offsetHSL((rnd() - 0.5) * 0.05, 0, (rnd() - 0.5) * 0.12);
      peces.setColorAt(i, col);
    }
    escena.add(peces);
